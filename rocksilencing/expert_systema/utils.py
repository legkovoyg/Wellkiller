from expert_systema.models import Technology
...
def filter_technologies(rock_type, pressure_coefficient, temperature, is_water_sensitive):
    ...
    base_query = Technology.objects.filter(**{collector_field: 2})  # Фильтрация по коллектору
    ...
    if pressure_field:
        base_query = base_query.filter(**{pressure_field + '__in': [1, 2]})  # Применим или частично применим

    # Ранжирование технологий
    applicable_technologies = base_query.filter(**{pressure_field: 2})  # Применим
    partially_applicable_technologies = base_query.filter(**{pressure_field: 1})  # Частично применим
