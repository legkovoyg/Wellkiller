...
def reservoir_characteristics_view(request):
    ...
    """
    # Получаем design_id из GET или POST запроса
    design_id = request.GET.get("design_id") or request.POST.get("design_id")
    design_obj = None
    if design_id:
        try:
    ...
    if request.method == "POST":
        ...
        if "submit_select_tech" in request.POST:
            # Обработка формы выбора группы технологий и технологии
            ...
            if calc_type == "Экспертная система":
                redirect_url = reverse("filesapp:files_main_window")
            elif calc_type in [
                "Глушение скважины c экспертной системой",
                "Всё включено",
            ]:
                redirect_url = reverse("calculator:home")
            else:
                # Значение по умолчанию
                redirect_url = reverse("filesapp:files_main_window")

            # Добавляем design_id к параметрам перенаправления, если он существует
            if design_id:
                query_params = urlencode({"design_id": design_id})
                redirect_url = f"{redirect_url}?{query_params}"

            return redirect(redirect_url)

        elif "submit_reservoir" in request.POST:
            # Обработка основной формы характеристик коллектора
            ...
            if form.is_valid():
                ...
                for group_name, tech_list in (grouped_results or {}).items():
                    ...
                    for tech_obj, applicability in tech_list:
                        ...
                        if components_data["polymers"]:
                            polys = [
                                poly["name"] for poly in components_data["polymers"]
                            ]
                        ...
                        if components_data["other_materials"]:
                            mats = [
                                mat["name"]
                                for mat in components_data["other_materials"]
                            ]
                            components_text.append(
                                f"Прочие компоненты: {', '.join(mats)}"
                            )
                ...
                return render(
                    ...
                    "reservoir_characteristics_form.html",
                    {
                        ...
                        "selected_technology": request.session.get(
                            "selected_technology"
                        ),
                        "design_id": design_id,  # Передаем design_id в контекст
                    },
                )

            # Если форма не валидна, рендерим форму с ошибками
            return render(
                request,
                "reservoir_characteristics_form.html",
                {
            ...
            # Обработка POST запроса без распознаваемой формы
            # Можно вернуть сообщение об ошибке или перенаправить
            return render(
                request,
                "reservoir_characteristics_form.html",
                {
